...
public class GalleryNodeRenderer implements NodeRenderer {
	...
	public GalleryNodeRenderer(HtmlNodeRendererContext context, String servletContextPath, int w, int h) {
	...
	public void render(Node node) {
		...
		for (String img : ((GalleryBlock) node).getImgs()) {
			writer.tag("a",
					Collections.singletonMap("href", servletContextPath + StringUtils.prependIfMissing(img, "/img")));
			writer.tag("img", Collections.singletonMap("src",
					servletContextPath + StringUtils.prependIfMissing(img, "/img") + "?w=" + w + "&h=" + h));
			writer.tag("/a");
		}
		writer.tag("/div");
	}
